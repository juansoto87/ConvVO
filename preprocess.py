...
parser.add_argument("--data_pose_dir", type=str, required=True, help="Where the pose dataset is stored")
...
def concat(folder, destination):
    ...
    for f in folder:
        ...
        data = np.load(os.path.join(args.data_pose_dir, (f+".npy")))
        ...
        for i in range(len(image_list)-1):
            ...
            np_file = np.append(np_file, [[(data[i + 1, 0] - data[i, 0]), (data[i + 1, 1] - data[i, 1]),
                          (data[i + 1, 2] - data[i, 2]), (data[i + 1, 3] - data[i, 3]),
                          (data[i + 1, 4] - data[i, 4]), (data[i + 1, 5] - data[i, 5])]], axis=0)
            img_concat = get_concat_h(img1,img2).save(os.path.join(destination,(f + "_" + image_list[i])))
    np.save(os.path.join(destination,'poses'), np_file)
    final_list=[]
...
def chec_npy(main_folder,file):
    data = np.load(os.path.join(main_folder,(file+".npy")))
    for i in range(20):
        print(data[i])
